#!/usr/bin/env python3
"""Week 5 — DLA growth vs CuSO4 concentration (0.02 / 0.04 / 0.06).
...
FIGS = ROOT / "figures"
OVER = ROOT / "overlays"
FRAMES_DIR = Path(os.environ.get("W5_FRAMES", "/tmp/w5"))
TMP = Path(os.environ.get("W5_TMP", "/tmp/w5"))
...
RUNS = [
    dict(tag="run1", conc=0.02, label="run 1 — 0.02"),
    dict(tag="run2", conc=0.04, label="run 2 — 0.04"),
    dict(tag="run3", conc=0.06, label="run 3 — 0.06"),
]
...
def mm_per_px(ref_bgr, strip_w=None, diag_path=None):
    ...
    col_score = (blueness > 15).mean(axis=0)
    ...
    ac /= ac[0] + 1e-9
    # first local max after the zero-lag dip, searched in a plausible range
    lo, hi = 8, 80
    seg = ac[lo:hi]
    ...
    if 1 <= k < len(ac) - 1:
        ...
        denom = (y0 - 2 * y1 + y2)
        k = k + (0.5 * (y0 - y2) / denom if abs(denom) > 1e-12 else 0.0)
    if diag_path:
...
def measure_run(run, k0=0, k1=None):
    ...
    for k in range(k0, k1):
        ...
        if len(xs) < 5:
            rows.append((k, t, 0, 0, 0, 0, 0, 0, 0, int(raw.sum()), dx, dy))
            continue
...
def fractal_run(run, thr_scan=(0.10, 0.15, 0.20)):
    ...
    Ds = [r[0] for hi in thr_scan
          if (r := _bc(cluster_gate(deposit_mask(f, ref, hi=hi, lo=hi * 0.4),
                                    seed))) is not None]
    ...
    m = cluster_gate(deposit_mask(f, ref), seed)
    ...
    w = (M > 0.08 * Mref) & (M < 0.75 * Mref) & (cl == 0) & (Rg > 0)
...
def radius_figure():
    ...
    for i, run in enumerate(RUNS):
        ...
        good = (R > 0) & (cl == 0) & hampel_inliers(R)
        clipped = (R > 0) & (cl == 1)
...
def main():
    ...
    ap.add_argument("--runs", default="run1,run2,run3")
